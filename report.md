...
**Overview-first 多视图联动**：Shneiderman (1996) 提出的 Visual Information Seeking Mantra 是可视化设计的经典指导原则："Overview first, zoom and filter, then details-on-demand"。本系统严格遵循这一原则——热力图提供 1880–2025 全时段 overview（8 行 × 146 列），3D 散点提供单年 detail（8 个高亮 point + 5 年尾迹），CO₂ 时序提供趋势 context。三面板通过统一时间滑块同步，遵循 Heer et al. (2010) 在 Visualization Zoo 中强调的联动多视图原则：各视图展示数据的不同切面，用户在与一个视图交互时无需手动维护跨视图的时间一致性。滑块为单一真相源（single source of truth）——所有面板共享同一时间状态，避免多控制器间的协调负担。
...
2. 3D 散点不可避免存在遮挡，虽已通过颜色冗余编码缓解，但仍依赖用户手动旋转视角。可增加正交投影选项或预设视角快捷键。
