...
其中 T1–T3 为核心任务，直接对应三个研究问题；T4 支持探索式假设生成；T5–T6 支持结论验证。T2 通过热力图的全局对比间接支持——用户可在 3D 散点中观察各纬度带的空间分布差异，并在热力图中对比不同十年的色块模式。
...
![3D 气候仪表板截图占位图](screenshots/index-placeholder.png)

主可视化采用 item-set 联动多视图布局，三个面板通过年份滑块同步。核心是 3D 散点图——每个 point 代表某年某纬度带的一次观测，47 帧逐年推进。当前帧为高亮不透明 point，背景层始终以半透明 point 展示全部 47 年数据作为参照，动画仅切换高亮帧，使用户能对比当前年份与全局分布。这种 temporal encoding 使用户能感知数据点随时间的位移方向和速度：北极点快速向高温区移动，直观传达极地放大效应。
...
|---------|-------------|------------|
| 年份滑块 | Navigate (sequence) | 控制时间帧，面板同步 |
...
| 双击重置视角 | Navigate (viewpoint) | 从任意视角回到三维全景 |

设计逻辑：Navigate 类交互（滑块、播放、旋转、预设视角）服务探索式浏览（T1/T4），Select 类（悬停）服务精确识别（T3）。预设视角功能通过正交投影将三维结构分别投影到三个二维平面，降低 3D 交互门槛。
...
**温度的颜色冗余编码**：Munzner (2014) 指出在通道带宽允许时，冗余编码可增强读图效率。3D 视角旋转时不可避免会产生遮挡，被前景点挡住的背景点其 Z 轴信息丢失。通过 RdBu_r 发散色板将温度同时映射到颜色通道，用户在旋转视角后仍可通过颜色感知温度高低，无需反复调整视角。这种冗余设计在 3D 可视化中尤为重要——它补偿了 3D 投影不可避免的深度信息损失。

**发散色板而非彩虹色**：Borland & Taylor (2007) 系统论证了彩虹色阶的三个缺陷：(1) 感知非线性——等距数据不产生等距感知差异；(2) 人为边界——色相急剧变化处易被误读为数据边界；(3) 对色觉障碍者不友好。温度异常数据天然是 diverging 类型——正值为偏暖（红），负值为偏冷（蓝），零值为基准。RdBu_r 发散色板将零值映射为白色中点，正负方向各沿单一色相渐变，满足感知线性度要求，语义直观（红=热、蓝=冷），且对红绿色盲友好。

**动画与静态视图互补**：Tversky et al. (2002) 通过综述研究指出动画在展示连续变化过程时比静态序列更有效，但不利于精确数值比较——用户难以同时追踪多个运动对象的具体数值。基于这一发现，本设计采用双轨策略：3D 散点的逐年动画用于展示时间演化趋势和运动轨迹（服务 T1 Discover 和 T4 Browse），静态温度热力图覆盖 146 年全局提供精确对比基线（服务 T3 Identify 和 T6 Summarize），CO₂ 时序线提供量化的时间上下文（服务 T5 Locate）。用户在热力图中定位异常年份，再到 3D 视图中观察其空间结构——两视图互补而非重复。

**Overview-first 多视图联动**：Shneiderman (1996) 提出的 Visual Information Seeking Mantra 是可视化设计的经典指导原则："Overview first, zoom and filter, then details-on-demand"。本系统严格遵循这一原则——热力图提供 1880–2025 全时段 overview（8 行 × 146 列），3D 散点提供单年 detail（8 个高亮 point + 全部 47 年历史数据作为背景参照），CO₂ 时序提供趋势 context。三面板通过统一时间滑块同步，遵循 Heer et al. (2010) 在 Visualization Zoo 中强调的联动多视图原则：各视图展示数据的不同切面，用户在与一个视图交互时无需手动维护跨视图的时间一致性。滑块为单一真相源（single source of truth）——所有面板共享同一时间状态，避免多控制器间的协调负担。
...
**CO₂ 敏感性存在显著纬度梯度**： 3D 散点图中北极带数据点随 CO₂ 升高快速向 Z 轴正方向（高温区）移动，而赤道带数据点的垂直位移明显较小。热力图中北极带在 2000 年后几乎全部呈现深红色，赤道带则多为浅橙至白色。CO₂ 从 336.85 ppm 升至 425.65 ppm（+89 ppm），北极温度异常从约 −0.6°C 升至 +3.01°C，赤道带仅从约 +0.3°C 升至 +0.9°C 左右。这表明全球平均升温数字严重低估了极地地区的实际升温幅度。
...
**优点**： 所有用户均能顺利完成两个任务。热力图被评为最直观的视图，无需学习即可理解色标含义。3D 散点图的播放动画获得一致好评，用户表示"能清晰看到北极点的运动轨迹"。多视图联动（滑块同步面板）减少了手动对齐负担。
...
2. 3D 散点不可避免存在遮挡，虽已通过颜色冗余编码和预设正交投影视角缓解，但仍需用户主动切换视角。可进一步增加自动推荐最佳视角的功能。
